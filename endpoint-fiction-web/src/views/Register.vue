<template>
  ...
  <div class="main box_center cf">
    <div class="userBox cf">
          ...
          <ul class="log_list">
            ...
            <li class="log_code cf">
              <input
                ...
                placeholder="请输入验证码"
              />
              <el-button class="code_pic"  @click="send">{{ countDown }}</el-button>
              <!-- <img
      ...
      <div class="user_r">
        <p class="tit">已有账号？</p>
        <!-- <router-link to="/login" class="btn_ora_white"
          >立即登录</router-link
        > -->
        <a>立即登录</a>    
        <!--
...
import { sendSmsCode, register  } from "@/api/member";
...
export default {
  ...
  setup() {
    ...
    const send =  async ()=> {
      if (!/^\d{11}$/.test(state.username)){
        ElMessage('请输入正确的手机号!');
        return;
      }
      // 判断bVerification false没发送过
      if(!state.bVerification){
         //调用方法发送手机验证码  
         const result = await sendSmsCode({phone:state.username});
         console.log("验证码内容11",result)
        if(result.code==200){
          ElMessage({
            message: '验证码已发送到您的手机中',
            type: 'success',
          })
        ...
        let countDown = 59;
            const auth_time = setInterval(() => {
              state.countDown = (countDown--)+"秒再发送";
                if (countDown <= 0) {
