            ...
            >
              <div class="book_title">
                <h1 v-if="data.chapterInfo">
                  {{ data.chapterInfo.indexName }}
                </h1>
                <div class="textinfo" v-if="data.chapterInfo">
                  类别：{{ data.bookInfo.catName }} 作者：<a
                    href="javascript:searchByK('最终马甲')"
                    v-if="data.bookInfo"
                    >{{ data.bookInfo.authorName }}</a
                  ><span v-if="data.chapterInfo"
                    >字数：{{ data.chapterInfo.wordCount }}</span
                  ><span v-if="data.chapterInfo"
                    >更新时间：{{ data.chapterInfo.updateTime }}</span
                  >
                </div>
              </div>

              <div class="txtwrap" v-if="!data.needBuy" >
                <div  
                  id="showReading"
                  class="readBox"
              ...
              <div class="txtwrap" v-else>
              <div id="showBooking" class="orderBox">
                  <h3>此章为VIP章节，需要订阅后才能继续阅读</h3>
                      <ul class="order_list">
                          <li>价格：<span class="red" > {{data.chapterInfo.bookPrice}} 终点币</span></li>
                          <li id="panelPay" class="btns"  ><a class="btn_red" @click="buyBookIndex"  >购买本章</a></li>
                      </ul>
              </div>
            </div>
...
import { buyBookByIndexId,addReadHistory } from "@/api/member";
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      if(getToken){
        loggedAddReadHistory(route.params.id,route.params.chapterId);
      }
      keyDown();
    });
    ...
    const preChapter = async (bookId) => {
      ...
      if (data) {
        ...
        window,scrollTo(0,0);
        if(getToken){
          loggedAddReadHistory(bookId,data);
        }
      } else {
    ...
    const loggedAddReadHistory =async(bookId,chapterId)=>{
      console.log("bookId:",bookId,"chapterId:",chapterId);
        const res = await addReadHistory({bookId:bookId,preIndexId:chapterId});
        if(res.code ==200){
    ...
    const buyBookIndex =  async()=>{
     const res = await  buyBookByIndexId(route.params.chapterId);
     console.log("解锁章节结果响应为:",res);
     if(res.code==200){
      ElMessage.success("恭喜您解锁成功!");
      init(route.params.chapterId);
     }else{
