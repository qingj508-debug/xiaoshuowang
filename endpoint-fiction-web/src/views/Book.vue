                ...
                <li>
                  <span class="fl font16">
                    <a
                      ...
                      >{{ chapterAbout.indexName }}】</a
                    ></span
                  >
                  <span class="black9 fr" v-if="chapterAbout.updateTime"
                    >更新时间：{{
                      chapterAbout.updateTime
                    }}</span
                  >
                </li>
                ...
                >
                  ...
                  <ul class="pl_bar fr">
                    ...
                    <li class="other cf">
                      ...
                      ><span class="fr" v-if="item.createUserId == uid"
                        ><a
                          href="javascript:void(0);"
                          @click="
...
import {isAddToBookshelf ,addToBookshelf } from "@/api/member";
...
export default {
  ...
  setup() {
    ...
    const checkisAddToBookshelf = async(bookId)=>{
      const res= await  isAddToBookshelf(bookId);
      console.log("查询书籍是否被加入过书架",res);
      if(res.code==200&&res.data){
        isbookshelf.value= true;
      }
    }

    const addFavorites = async(bookId)=>{
       const res = await addToBookshelf({bookId:bookId,preIndexId:0});
       if(res.code==200){
        isbookshelf.value= true;
        ElMessage.success("成功加入书架")
       }
    }
    const addVisitCount = async(bookId)=>{
      if(getToken){
        addBookVisitCount(bookId);
      }
    }
