        ...
        <ul class="list">
          <li class="so_pd" id="workDirection">
            ...
            <a
              ...
              :class="`${state.workDirectionOn == 0 ? 'on' : ''}`"
              >男频</a
            >
            <a
              filter-value="1"
              ...
              :class="`${state.workDirectionOn == 1 ? 'on' : ''}`"
              >女频</a
            >
          </li>
          <li id="idGirl" class="so_class">
            ...
            <span class="so_boy" id="boyCategoryList">
              <a
                href="javascript:void(0)"
                :class="`${state.categoryOn == 0 ? 'on' : ''}`"
                @click="changeCategory(0)"
                >不限</a
              >
              <a
                v-for="(item, index) in state.bookCategorys"
                :key="index"
                href="javascript:void(0)"
                :class="`${state.categoryOn == item.id ? 'on' : ''}`"
          ...
          <li class="so_progress">
            ...
            <a
              href="javascript:void(0)"
              :class="`${state.bookStatusOn == null ? 'on' : ''}`"
          ...
          <li class="so_words">
            ...
            <a
              href="javascript:void(0)"
              :class="`${state.wordCountOn == null ? 'on' : ''}`"
            ...
            <a
              ...
              href="javascript:void(0)"
              :class="`${state.wordCountOn == 300000 ? 'on' : ''}`"
            ...
            <a
              ...
              href="javascript:void(0)"
              :class="`${state.wordCountOn == 500000 ? 'on' : ''}`"
            ...
            <a
              ...
              href="javascript:void(0)"
              :class="`${state.wordCountOn == 1000000 ? 'on' : ''}`"
          ...
          <li class="so_update">
            ...
            <a
              href="javascript:void(0)"
              :class="`${state.updateTimeOn == null ? 'on' : ''}`"
            ...
            <a
              filter-value="15"
              href="javascript:void(0)"
              :class="`${state.updateTimeOn == 15 ? 'on' : ''}`"
            ...
            <a
              filter-value="30"
              href="javascript:void(0)"
              :class="`${state.updateTimeOn == 30 ? 'on' : ''}`"
          ...
          <li class="so_sort">
            ...
            <a href="javascript:void(0)"
              :class="`${state.sortOn == null ? 'on' : ''}`"
              @click="changeSort(null)">不限</a>
            <a
              filter-value="last_index_update_time"
              href="javascript:void(0)"
              :class="`${state.sortOn == 'lastIndexUpdateTime_desc' ? 'on' : ''}`"
              @click="changeSort('lastIndexUpdateTime_desc')"
              >更新时间</a
            >
            <a filter-value="word_count" href="javascript:void(0)"
              :class="`${state.sortOn == 'wordCount_desc' ? 'on' : ''}`"
              @click="changeSort('wordCount_desc')"
              >总字数</a
            >
            <a
              filter-value="visit_count"
              href="javascript:void(0)"
              :class="`${state.sortOn == 'visitCount_desc' ? 'on' : ''}`"
              @click="changeSort('visitCount_desc')"
              >点击量</a
            >
          </li>
        </ul>
    ...
    <div class="channelWrap channelClassContent cf">
      <div class="updateTable rankTable">
        <table cellpadding="0" cellspacing="0">
          ...
          <tbody id="bookList">
            <tr v-for="(item, index) in state.books" :key="index">
              <td class="rank">
                <i>{{ index + 1 }}</i>
              </td>
              <td class="style">
              ...
              <td class="chapter">
                <a href="javascript:void(0)" @click="bookDetail(item.id)" v-html=" item.lastIndexName" ></a>
              </td>
              <td class="author">
        ...
        <el-pagination
          ...
          background
          :page-size="state.searchCondition.pageSize"
          :total="state.total"
          class="mt-4"
...
import Navbar from "@/components/common/NavbarView";
    ...
    const loadCategoryList = async (workDirection) => {
      ...
      state.categoryOn = 0;
      state.searchCondition.catId = null;
      search();
    };

    const changeCategory = async (catId) => {
      state.categoryOn = catId;
      if (catId > 0) {
        state.searchCondition.catId = catId;
      } else {
        state.searchCondition.catId = null;
      }
      search();
    ...
    const wordCountFormat = computed(() =>{
       return function(wordCount){
        wordCount+="";//变为string方便判断位数
...
.el-pagination {
  --el-pagination-hover-color: #FF0000 !important;
}
</style>
